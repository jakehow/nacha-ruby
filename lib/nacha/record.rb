...
module Nacha
  class Record
    ...
    def self.parse(_input, skip_validation: true)
      raise Error, "Parse must be implemented in subclass of Record"
    end

    def validate!
      raise ::Nacha::Error, "#{self.class} Record Invalid" unless validate
    end
  end
end
